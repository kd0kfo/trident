...
def validate_file(infile):
    ...
    for score in p:
        if not score:
            raise parser.BrokenLine("Broken score line in {0}"
                                    .format(infile.name))
        score_counter += 1
        ...
        for type_name in dir(core):
            if "MATCH" in type_name:
                if (type_name in ["MATCH_DIRECT_REVERSE_HOOGSTEEN",
                                  "MATCH_INDIRECT_REVERSE_HOOGSTEEN"]
                        and score['base_type'] == "pyrimidine"):
                    continue
                elif (type_name in ["MATCH_DIRECT_HOOGSTEEN",
                                    "MATCH_INDIRECT_HOOGSTEEN"]
                      and score['base_type'] == "purine"):
                    continue

                energy = sequence_energy(query, reference,
                                         core.__getattribute__(type_name))
                energy = round(energy, 3)

                if verbose:
                    print("Energy (%s,%s,%s): %f" % (query, reference,
                                                     type_name, energy))
